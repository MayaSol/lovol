...
function writeSassImportsFile(cb) {
  ...
  newScssImportsList.push('src/css/scss-imports.scss');
...
function buildStyleImports() {
  const entryList = {
    'scss-imports': `./${dir.src}scss/scss-imports.js`,
  };
  console.log(entryList);
  return src(`${dir.src}scss/scss-imports.js`)
    .pipe(plumber({
      errorHandler: function(err) {
        console.log(err.message);
        this.emit('end');
    ...
    .pipe(webpackStream({
      ...
      plugins: [
        new MiniCssExtractPlugin({
          filename: "scss-imports.scss",
        })
      ],
    }))
    .pipe(dest(`${dir.src}css`));
}
exports.buildStyleImports = buildStyleImports;
